...
const OnboardingPopulate = (() => {
  function populateOnboardingMemory(scanResults) {
    ...
    const skipContent = new Set(
      (mem.entries || []).map((e) => (typeof e === 'string' ? e : e.content || '')),
    );
    const pending = [];
  ...
  return { populateMemory: populateOnboardingMemory, populateHandoff: populateOnboardingHandoff };
})();

// Convenience shims so onboarding.js can call without namespace
const populateOnboardingMemory = (scanResults) => OnboardingPopulate.populateMemory(scanResults);
const populateOnboardingHandoff = (scanResults) => OnboardingPopulate.populateHandoff(scanResults);
